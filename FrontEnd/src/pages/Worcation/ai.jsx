...
// Pagination for AI list is kept locally in this component (no store needed)
...
const WorcationAiPage = () => {
  ...
  // AI 추천 페이지 데이터 로드 (로컬 페이징)
  useEffect(() => {
    const fetchAi = async () => {
      if (!loginUser) return;
      setAiLoading(true);
      try {
        const aiResponse = await worcationService.getAIList(currentPage, pageSize);
        const aiData = aiResponse.content || [];
        setAiWorcations(Array.isArray(aiData) ? aiData : aiData ? [aiData] : []);
        // API returns snake_case pagination fields
        setTotalPages(aiResponse.total_page || 0);
      } catch (err) {
    ...
    fetchAi();
  }, [loginUser, currentPage, pageSize]);

  // 페이지네이션 제거됨 — 더 이상 페이지 변경 로직 없음
