...
const HostSchema = yup.object().shape({
  ...
  open_date: yup.date().required('개업일 선택...'),
...
export const useWorcationForm = () => {
  ...
  const {
    ...
    resolver: yupResolver(HostSchema), // yup스키마와 연결
    mode: 'onChange',
  });
  ...
  const onSubmit = async (data) => {
    try {
      ...
      */
      ...
      await worcationService.insert(user.user_no, {
        licensee: data.licensee,
        worcation_name: data.worcation_name,
        open_date: data.open_date,
        business_id: data.business_id,
