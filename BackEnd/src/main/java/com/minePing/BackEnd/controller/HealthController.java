...
public class HealthController {
    ...
    /**
     * AI 기반 식단 추천 (기존 ChatController의 diet-recommend를 HealthController로 통합)
     * 원래 경로: POST /api/v1/ai/diet-recommend/{userNo}
     * 통합 경로: POST /api/v1/health/ai/diet-recommend/{userNo}
     */
    @PostMapping("/ai/diet-recommend/{userNo}")
    public Map<String, Object> recommendDiet(@PathVariable Long userNo) {
        Map<String, Object> response = new HashMap<>();

        List<Map<String, String>> meals = null;
        try {
            AiDto.AiEat dto = healthService.getAiEatByUser(userNo);
